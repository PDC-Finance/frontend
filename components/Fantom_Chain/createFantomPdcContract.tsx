...
const CreateFantomPdcContaract = ({
 address,
  setIsAvailablePDC,
  ...
  try {
    ...
    useContractRead({
      addressOrName: process.env.NEXT_PUBLIC_FANTOM_PDC_FACTORY_CONTRACT_ADDRESS ? process.env.NEXT_PUBLIC_FANTOM_PDC_FACTORY_CONTRACT_ADDRESS : "",
  ...
  return (
    <>
      <button disabled={!write} onClick={() => write?.()}  className="bg-green-500 px-10 py-2 rounded-xl text-white font-bold hover:bg-green-700">
        Create Fantom PDC Contract
      </button>
    </>
  );
};

export default CreateFantomPdcContaract;
